...
fn main() {
    input! {
        n: usize,
        m: usize,
        abcd: [(usize, usize, i64, u8); m]
    };

    let mut graph: Vec<Vec<(usize, i64)>> = vec![Vec::new(); n];
    let mut edges: HashMap<usize, HashMap<usize, (i64, u8)>> = HashMap::new();
    for &(a, b, c, d) in abcd.iter() {
        let a = a - 1;
        let b = b - 1;

        graph[a].push((b, 10000 * c - d as i64));
        graph[b].push((a, 10000 * c - d as i64));

        let entry = edges.entry(a).or_insert(HashMap::new());
        entry.insert(b, (c, d));

        let entry = edges.entry(b).or_insert(HashMap::new());
        entry.insert(a, (c, d));
    }
    ...
    for (from, to) in path.iter().tuple_windows() {
        let (weight, tree) = edges.get(from).unwrap().get(to).unwrap();
        distance += *weight;
        trees += *tree as i64;
    }

    println!("{} {}", distance, trees);
}
